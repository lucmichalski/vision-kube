...
LICENSE_KEY="6ea719e8-f9a2-4099-b6d2-6f8f22dd93d0"
/vmx/middle/vmx&
sleep 10
VMX_HOST=$(hostname -f)
...
curl -X POST -d '{"email":"luc.michalski@blippar.com"}' $VMX/activate/6ea719e8-f9a2-4099-b6d2-6f8f22dd93d0
...
UUID=`curl -s $VMX/models | jq -r '.data[] | select(.name=="'$NAME'") .uuid'`
echo "====== His Unique Identifier is $UUID"

if [ -z $UUID ]; then
    echo ""
    echo "Cannot find model named" $NAME
    echo "Available models:" `curl -s $VMX/models | jq -r '.data[] .name'`
    echo ""
else

    echo "====== List of sessions already there: $SESSIONID"
    # Start a new session if we didn't find one
    echo "curl -s -X POST -d '{"uuids":["'$UUID'"]}' $VMX/sessions | jq -r .data.id"
    SESSIONID=`curl -s -X POST -d "{\"uuids\":[\""$UUID"\"]}" $VMX/sessions | jq -r .data.id`
...
MODELS=`curl -s $VMX/models | jq -r '.data[] .name'`
IFS=$'\n'
