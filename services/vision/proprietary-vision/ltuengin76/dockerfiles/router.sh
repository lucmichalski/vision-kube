...
#               echo -e "    [backends.backend-$MODEL_NAME.circuitbreaker]" >> $TEMP_BACKEND_FILE
#               echo -e '      expression = "NetworkErrorRatio() > 0.5 AND LatencyAtQuantileMS(50.0) > 50 AND ResponseCodeRatio(500, 600, 0, 600) > 0.5"' >> $TEMP_BACKEND_FILE
            fi
...
#               echo -e "    [frontends.frontend-"$PORT".routes.path_$PORT]" >> $TEMP_FRONTEND_FILE
#               echo -e "    rule = \"Path\"" >> $TEMP_FRONTEND_FILE
#               echo -e "    value = \"/\"" >> $TEMP_FRONTEND_FILE
               echo -e "    [frontends.frontend-"$PORT".routes.lookup_"$PORT"]" >> $TEMP_FRONTEND_FILE
               echo -e "    rule = \"PathPrefix\"" >> $TEMP_FRONTEND_FILE
               echo -e "    value = \"/"$MODEL_NAME"\"" >> $TEMP_FRONTEND_FILE
            fi
